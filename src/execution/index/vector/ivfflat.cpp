...
unique_ptr<IndexScanState> IvfflatIndex::InitializeScanSinglePredicate(const Transaction &transaction,
                                                                       const Value &value,
...
bool IvfflatIndex::ScanWithBindData(Transaction &transaction, DataTable &table, TableScanBindData &bind_data, bool filtered) {
  ...
  std::cout << "ivfflatindex_scanwithbinddata: " << bind_data.result_ids.size() << " filtered: "  << filtered <<  std::endl;
...
PreservedError IvfflatIndex::Insert(IndexLock &lock, DataChunk &input, Vector &row_ids) {
	...
	auto vector_data_ptr =
	    reinterpret_cast<float *>(arena_allocator.AllocateAligned(input.size() * dimension * v_size));
	...
	for (idx_t i = 0; i < input.size(); i++) {
		...
		for (idx_t j = 0; j < entry.length; j++) {
			...
			auto idx = real_data.sel->get_index(offset);
			memcpy(vector_data_ptr + values_count, data_ptr + idx, v_size);
			values_count += 1;
		}
	}
